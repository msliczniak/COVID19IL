...
# This version of ndiff is heavily based on the version distributed with
# python 2.1.1. It has been slightly modified so that it works with wr and
# family.
...
confusing if the source files contain tab characters. If -t is specified, n
specifies the tab stop, and output will be tabified with spaces according to the
tab stop value. In that case, output lines beginning with ? will treat tabs with
a visual length identical to the tab stop value.
...
def dump(tag, x, lo, hi, ts):
    if ts:
        for i in xrange(lo, hi):
            print tag, x[i].expandtabs(ts),
    else:
        for i in xrange(lo, hi):
            print tag, x[i],

def plain_replace(a, alo, ahi, b, blo, bhi, ts):
...
def fancy_replace(a, alo, ahi, b, blo, bhi, ts):
    if TRACE:
    ...
    for j in xrange(blo, bhi):
        bj = b[j]
        if ts:
            cruncher.set_seq2(bj.expandtabs(ts))
        else:
        ...
        for i in xrange(alo, ahi):
            ...
            if ai == bj:
                ...
                continue
            if ts:
                cruncher.set_seq1(ai.expandtabs(ts))
            else:
    ...
    if best_ratio < cutoff:
        ...
        if eqi is None:
            ...
            plain_replace(a, alo, ahi, b, blo, bhi, ts)
            return
    ...
    if TRACE:
        ...
        dump('>', a, best_i, best_i+1, ts)
        dump('<', b, best_j, best_j+1, ts)

    # pump out diffs from before the synch point
    fancy_helper(a, alo, best_i, b, blo, best_j, ts)
    ...
    if eqi is None:
        ...
        printq(aelt, belt, atags, btags, ts)
    else:
        # the synch pair is identical
        print ' ', aelt,

    # pump out diffs from after the synch point
    fancy_helper(a, best_i+1, ahi, b, best_j+1, bhi, ts)

def fancy_helper(a, alo, ahi, b, blo, bhi, ts):
    if alo < ahi:
        if blo < bhi:
            fancy_replace(a, alo, ahi, b, blo, bhi, ts)
        else:
            dump('-', a, alo, ahi, ts)
    elif blo < bhi:
        dump('+', b, blo, bhi, ts)

def expandtabs(line, tags, ts):
    i = ts
    oline = ''
    otags = ''
    for c, t in map(None, line, tags):
	if c != "\t":
            oline = oline + c
            otags = otags + t
            if i != 1:
    ...
    if count_leading(otags, " ") < len(otags):
        print "?", otags

# Crap to deal with leading tabs in "?" output.  Can hurt, but will
# probably help most of the time.

def printq(aline, bline, atags, btags, ts):
    if ts:
        print "-",
        expandtabs(aline, atags, ts)
        print "+",
        expandtabs(bline, btags, ts)
    else:
...
def main(args):
    import getopt
    try:
        opts, args = getopt.getopt(args, "qr:t:")
    ...
    if rseen and tseen:
        return fail("can't specify both -r and -t")
    if rseen:
        if args:
    ...
    if tseen:
        try:
             ...
             return fail("-t takes a small integer")

        if tabstop < 0:
             return fail("-t cannot take a negative value")
    ...
    if noisy:
        print '-:', f1name
        print '+:', f2name
    return fcompare(f1name, f2name, tabstop)
